...
const ProductPagination = ({ currentPage, totalPages, handlePaginate }) => {
  const pageNumbers = [];
  ...
  return (
    <ul className="flex justify-center items-center space-x-2 my-4">
      ...
      {pageNumbers.map((number) => (
        <li key={number}>
          <button onClick={() => handlePaginate(number)} className={`text-gray-700 border-b-2 font-bold py-2 px-4 ${number === currentPage ? "border-green-500 text-green-500" : "border-none"}`}>
            {number}
          </button>
        </li>
      ))}
